import ../dataset, ../tensor/tensor, ../kernels, ../models/factorization_machine
import ../models/fm_base, ../extmath
from ../loss import newSquared
from ../regularizers/regularizers import newSquaredL21
...
proc perm(x: var seq[int]) =
  let n = len(x)
  for i in 0..<len(x):
    let pivot = i+rand(n-1-i)
    let tmp = x[i]
    x[i] = x[pivot]
    x[pivot] = tmp
