...
import regularizers/omegacs_slow
import models/fm_slow


suite "Test proximal block coordinate descent for OmegaCS":
  let
    n = 50
    d = 6
    nComponents = 4
        ...
        for fitLinear in [true, false]:
          for fitIntercept in [true, false]:
            for maxSearch in [0, 20]:
              ...
              checkAlmostEqual(fm.w, fmSlow.w, atol=1e-5)
              checkAlmostEqual(fm.P, fmSlow.P, atol=1e-5)
