...
suite "Test proximal stochastic gradient descent for L1":
  let
    n = 80
    d = 8
    nComponents = 4
        ...
        for fitLinear in [false, true]:
          for fitIntercept in [false, true]:
            ...
            var fmSlow = newFMSlow(
              ...
              fitLower = fitLower, fitLinear = fitLinear,
              fitIntercept = fitIntercept, randomState = 1)
            var sgdSlow = newPSGDSlow(maxIter = 5, tol = 0, reg=regSlow, gamma=0.0)
            ...
            var fm = newFactorizationMachine(
              ...
              fitIntercept = fitIntercept, randomState = 1)
            var sgd = newPSGD(
              maxIter = 5, verbose = 0, tol = 0, reg=reg, gamma=0.0
            )
            sgd.fit(X, y, fm)
