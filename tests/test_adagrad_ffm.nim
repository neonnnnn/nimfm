import unittest
import utils, optimizer/adagrad_ffm_slow
...
import nimfm/optimizer/adagrad_ffm
import model/ffm_slow


suite "Test AdaGrad for FFM":
  let
    n = 80
    d = 20
    nFields = 5
    ...
    for fitLinear in [false, true]:
      for fitIntercept in [false, true]:
        ...
        var ffmSlow = newFFMSlow(
          ...
          fitIntercept = fitIntercept, randomState = 1)
        var adagradSlow = newAdaGradSlow(maxIter = 5, tol = 0, shuffle=false)
        adagradSlow.fit(XMat, fieldDict, y, ffmSlow)
        
        # fit fast version
        var ffm = newFieldAwareFactorizationMachine(
          task = regression, nComponents = nComponents, fitLinear = fitLinear,
          fitIntercept = fitIntercept, randomState = 1)
        var adagrad = newAdaGrad( maxIter = 5, verbose = 0, tol = 0, shuffle=false)
        adagrad.fit(X, y, ffm)
  ...
  test "Test regularization":
    for fitLinear in [true, false]:
      for fitIntercept in [true, false]:
        ...
        adagradStrong.fit(X, y, ffmStrongReg)
